...
class MapRoundsNotifier<R : Round>(
    private val tagLogger: TagLogger?,
) : MapNotifier(), RoundsNotifier<R> {


    override suspend fun onRoundStarted(room: RoundedRoom<R>) {
        room.run {
    ...
    override suspend fun onRoundEnded(room: RoundedRoom<R>) {
        room.run {
